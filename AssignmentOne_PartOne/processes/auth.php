    ...
    public function receive_sign_up($MYSQL, $OBJ_SendMail, $conf, $lang){
        if(isset($_POST["signup"])){
            ...
            if (!filter_var($email_address, FILTER_VALIDATE_EMAIL)) {
                ...
                $data = array("email_address" => $email_address, 'token' => $token, 'token_expire' => $token_expire, 'token_valid' => 1);
                $table = "users";
                // $data = array("branchNo" => 'email_address', 'street' => 'token', 'city' => 'token_expire', 'postcode' => 'hello');
                // $table = "branch";
                $MYSQL->insert($table, $data);
                
                $replacements = array('fullname' => 'Buddy', 'site_name' => $conf["site_name"], 'token' => $token);
